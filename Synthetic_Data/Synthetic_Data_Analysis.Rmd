...
lppd <- lapply(models, function(x) {
  sapply(seq_along(test_period), function(ti){
    ...
    print(FILE)
    load(FILE)
    dat_test <- data_ordinal[year_id %in% test_period[ti], ] 
    log_lik_D <- extract(fit123, pars = "log_lik_D_test")[[1]]
    
    log_lik_S_with_NA <- log_lik_M_with_NA <- log_lik_F_with_NA <-
      matrix(0, ncol = ncol(log_lik_D), nrow = nrow(log_lik_D))
    
    log_lik_S <- extract(fit123, pars = "log_lik_R1_test")[[1]]
    log_lik_M <- extract(fit123, pars = "log_lik_R2_test")[[1]]
    log_lik_F <- extract(fit123, pars = "log_lik_R3_test")[[1]]
...
conditional_probabilities <- lapply(models, function(x) {
  sapply(seq_along(test_period), function(ti){
    ...
    print(FILE)
    load(FILE)
    dat_test <- dat[year_id %in% test_period[ti], ] 
...
res_BR <- sapply(conditional_probabilities, 
                  function(y) {
                    tab <- sapply(seq_along(test_period), function(ti){ 
                      lp <- y[[ti]]
                      D_test <-  dat[year_id %in% test_period[ti], "D"] 
