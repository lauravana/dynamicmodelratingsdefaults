...
transformed parameters {
  ...
  vector[T] delta; // original market factor
  ...
  delta = scale_cov_b(delta_raw, 1, chol_cor_delta);

  // ** Linear predictor **
  u_train = a[firm_id_train] + epsilon[1:N1];
  u_test  = a[firm_id_test]  - omega * b[year_id_test]  + epsilon[id_test];
  S =  beta0 + x_train * beta + u_train - omega * b[year_id_train];
  S1 =  x_train * beta1 + u_train + lambda[1] * delta[year_id_train];
  S2 =  x_train * beta2 + u_train + lambda[2] * delta[year_id_train];
  S3 =  x_train * beta3 + u_train + lambda[3] * delta[year_id_train];

  if (N2 != 0) {
    S_test  = beta0 + x_test * beta + u_test;
    S1_test =         x_test * beta + u_test + lambda[1] * delta[year_id_test];
    S2_test =         x_test * beta + u_test + lambda[2] * delta[year_id_test];
    S3_test =         x_test * beta + u_test + lambda[3] * delta[year_id_test];
  }
}
...
model {
  ...
  target += beta_lpdf(phideltastar | 20, 1.5);
  target += std_normal_lpdf(delta_raw);
